...
class WSFEv1(BaseWS):
    ...
    def inicializar(self):
        ...
        self.ImpTotal = self.ImpIVA = self.ImpOpEx = self.ImpNeto = self.ImptoLiq = (
            self.ImpTrib
        ) = None
        self.EmisionTipo = self.Periodo = self.Orden = ""
        self.FechaCbte = self.FchVigDesde = self.FchVigHasta = self.FchTopeInf = (
            self.FchProceso
        ) = ""
                ...
                "FeDetReq": [
                    {
                        "FECAEDetRequest": {
                            ...
                            "CondicionIVAReceptorId": f.get(
                                "condicion_iva_receptor_id"
                            ),
                            "PeriodoAsoc": (
                                {
                                    "FchDesde": f["periodo_cbtes_asoc"].get(
                                        "fecha_desde"
                                    ),
                                    "FchHasta": f["periodo_cbtes_asoc"].get(
                                        "fecha_hasta"
                                    ),
                                }
    ...
    def ParamGetTiposCbte(self, sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["CbteTipo"]).replace(
                "\t", sep
            )
    ...
    def ParamGetTiposConcepto(self, sep="|"):
        ...
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["ConceptoTipo"]
            ).replace("\t", sep)
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposDoc(self, sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["DocTipo"]).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposIva(self, sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["IvaTipo"]).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposMonedas(self, sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["Moneda"]).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposOpcional(self, sep="|"):
        ...
        return [
            (
                "%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["OpcionalTipo"]
            ).replace("\t", sep)
    ...
    def ParamGetTiposTributos(self, sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s\t%(FchDesde)s\t%(FchHasta)s" % p["TributoTipo"]).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetTiposPaises(self, sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s" % p["PaisTipo"]).replace("\t", sep)
            for p in res["ResultGet"]
        ]
    ...
    def ParamGetPtosVenta(self, sep="|"):
        ...
        return [
            (
                "%(Nro)s\tEmisionTipo:%(EmisionTipo)s\tBloqueado:%(Bloqueado)s\tFchBaja:%(FchBaja)s"
                % p["PtoVenta"]
            ).replace("\t", sep)
            for p in res.get("ResultGet", [])
        ]
    ...
    def ParamGetCondicionIvaReceptor(self, clase_cmp="A", sep="|"):
        ...
        return [
            ("%(Id)s\t%(Desc)s\t%(Cmp_Clase)s" % p["CondicionIvaReceptor"]).replace(
                "\t", sep
            )
            for p in res["ResultGet"]
        ]
